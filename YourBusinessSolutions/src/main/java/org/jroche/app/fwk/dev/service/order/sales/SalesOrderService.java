package org.jroche.app.fwk.dev.service.order.sales;
...
import org.jroche.app.fwk.dev.common.util.extjs.FilterRequest;
import org.jroche.app.fwk.dev.common.util.mapper.order.sales.SalesOrderItemsMapper;
import org.jroche.app.fwk.dev.common.util.mapper.order.sales.SalesOrderMapper;
...
import org.jroche.app.fwk.dev.persistence.model.product.Product;
import org.jroche.app.fwk.dev.persistence.model.user.Customer;
import org.jroche.app.fwk.dev.persistence.repo.order.sales.SalesOrderItemsRepository;
import org.jroche.app.fwk.dev.persistence.repo.order.sales.SalesOrderRepository;
import org.jroche.app.fwk.dev.persistence.repo.product.ProductRepository;
import org.jroche.app.fwk.dev.persistence.repo.user.UserRepository;
import org.jroche.app.fwk.dev.service.product.ProductService;
import org.jroche.app.fwk.dev.web.model.order.TransactionAmountSummary;
import org.jroche.app.fwk.dev.web.model.order.sales.SalesOrderItemsUI;
import org.jroche.app.fwk.dev.web.model.order.sales.SalesOrderUI;
...
public class SalesOrderService {
	...
	public SalesOrderUI create(SalesOrderUI uiBean) {
		...
		Customer user = userRepository.findByUsername(uiBean.getUserName());
		so.setUser(user);
	...
	public List<TransactionAmountSummary> monthlySales(int month, int year) {
		...
		String query = "select new org.jroche.app.fwk.dev.web.model.order.TransactionAmountSummary(day(orderDate), sum(total)) " 
				+ " from salesOrder where month(orderDate) = " + month + " and " + " year(orderDate) = " + year 
				+ " group by orderDate";
		
		return entityManager.createQuery(query).getResultList();
		
		/*String query = "select new org.jroche.app.fwk.dev.web.model.order.TransactionAmountSummary(year(orderDate), sum(total)) " +
				" from salesOrder " +
	...
	public List<TransactionAmountSummary> yearlySales(int year) {
		...
		String query = "select new org.jroche.app.fwk.dev.web.model.order.TransactionAmountSummary(month(orderDate), sum(total)) " 
